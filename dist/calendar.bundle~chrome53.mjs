 ...
 * @private
 */function friendlyDateTime(dateTimeish){if(DateTime$1.isDateTime(dateTimeish)){return dateTimeish;}else if(dateTimeish&&dateTimeish.valueOf&&isNumber$1(dateTimeish.valueOf())){return DateTime$1.fromJSDate(dateTimeish);}else if(dateTimeish&&typeof dateTimeish==="object"){return DateTime$1.fromObject(dateTimeish);}else{throw new InvalidArgumentError(`Unknown datetime argument: ${dateTimeish}, of type ${typeof dateTimeish}`);}}//@ts-nocheck
// =============================================================================
// Weekday
// =============================================================================
const ALL_WEEKDAYS=['MO','TU','WE','TH','FR','SA','SU'];class Weekday{constructor(weekday,n){if(n===0)throw new Error("Can't create weekday with n == 0");this.weekday=weekday;this.n=n;}static fromStr(str){return new Weekday(ALL_WEEKDAYS.indexOf(str));}// __call__ - Cannot call the object directly, do it through
...
RRule.FREQUENCIES=['YEARLY','MONTHLY','WEEKLY','DAILY','HOURLY','MINUTELY','SECONDLY'];RRule.YEARLY=Frequency.YEARLY;RRule.MONTHLY=Frequency.MONTHLY;RRule.WEEKLY=Frequency.WEEKLY;RRule.DAILY=Frequency.DAILY;RRule.HOURLY=Frequency.HOURLY;RRule.MINUTELY=Frequency.MINUTELY;RRule.SECONDLY=Frequency.SECONDLY;RRule.MO=Days.MO;RRule.TU=Days.TU;RRule.WE=Days.WE;RRule.TH=Days.TH;RRule.FR=Days.FR;RRule.SA=Days.SA;RRule.SU=Days.SU;RRule.parseString=parseString;RRule.optionsToString=optionsToString;function findIndex(list,value,cmp){if(!list.length){return 0;}let low=0,high=list.length-1,mid=0,cmpval=0;while(low<=high){mid=low+Math.floor((high-low)/2);cmpval=cmp(value,list[mid]);if(cmpval<0)high=mid-1;else if(cmpval>0)low=mid+1;else break;}if(cmpval<0)return mid;// insertion is displacing, so use mid outright.
else if(cmpval>0)return mid+1;return mid;}function compareDateTime(a,b){return a.toMillis()-b.toMillis();}function RRuleFrequencyFromString(frequency){switch(frequency){case'yearly':return RRule.YEARLY;case'monthly':return RRule.MONTHLY;case'weekly':return RRule.WEEKLY;case'daily':return RRule.DAILY;case'hourly':return RRule.HOURLY;case'minutely':return RRule.MINUTELY;case'secondly':return RRule.SECONDLY;default:throw new Error('invalid frequency.');}}function RRuleWeekDayFromString(day,nthOfPeriod=0){switch(day){case'mo':return nthOfPeriod?RRule.MO.nth(nthOfPeriod):RRule.MO;case'tu':return nthOfPeriod?RRule.TU.nth(nthOfPeriod):RRule.TU;case'we':return nthOfPeriod?RRule.WE.nth(nthOfPeriod):RRule.WE;case'th':return nthOfPeriod?RRule.TH.nth(nthOfPeriod):RRule.TH;case'fr':return nthOfPeriod?RRule.FR.nth(nthOfPeriod):RRule.FR;case'sa':return nthOfPeriod?RRule.SA.nth(nthOfPeriod):RRule.SA;case'su':return nthOfPeriod?RRule.SU.nth(nthOfPeriod):RRule.SU;default:throw new Error('invalid weekday.');}}class CalendarEvent{constructor(id,parentId,data,startTime,endTime,priority){this.parentId=null;this.id=id;this.parentId=parentId;this.data=data;this.start=startTime;this.end=endTime;const interval=Interval.fromDateTimes(startTime,endTime);this.duration=interval.toDuration();this.priority=priority;this._interval=interval;}get shortId(){return this.id.substring(0,7);}setPriority(priority){this.priority=priority;return this;}contains(dateTime){return this._interval.contains(dateTime);}getEvents(startTime,endTime){const interval=Interval.fromDateTimes(startTime,endTime);return interval.overlaps(this._interval)?[this]:[];}getEventsForDay(date){const startTime=date.startOf('day');const endTime=date.endOf('day');const interval=Interval.fromDateTimes(startTime,endTime);return interval.overlaps(this._interval)?[this]:[];}get interval(){return this._interval;}toJSON(){return{id:this.id,parentId:this.parentId,start:this.start.toISO(),duration:this.duration.toISO(),priority:this.priority,data:this.data};}static fromJSON(json){const startTime=DateTime$1.fromISO(json.start);const endTime=startTime.plus(Duration.fromISO(json.duration));const event=new CalendarEvent(json.id,json.parentId,json.data,startTime,endTime,json.priority);return event;}}class CalendarEventSeries extends CalendarEvent{constructor(id,parentId,data,startTime,endTime,priority,recurrence){super(id,parentId,data,startTime,endTime,priority);this.recurrence=recurrence;}contains(dateTime){//console.log('contains', dateTime.toISO());
if(dateTime<this.start){return false;}if(dateTime<this.end){return true;}const events=this.getEventsForDay(dateTime);//let i = 1;
for(const event of events){//console.log(i++, event.toJSON());
if(event.contains(dateTime)){return true;}}return false;}getEvents(startTime,endTime){//console.log(`getEvents(${startTime.toISO()},${endTime.toISO()})`);
const events=[];const iterator=dateTime=>{const event=new CalendarEvent(this.id,this.parentId,this.data,dateTime,dateTime.plus(this.duration),this.priority);events.push(event);};this._getEvents(startTime,endTime,iterator);//console.log("events =", JSON.stringify(events));
return events;}getEventsForDay(date){console.log(`getEventsForDay(${date.toISO()})`);const events=[];const iterator=dateTime=>{const event=new CalendarEvent(this.id,this.parentId,this.data,dateTime,dateTime.plus(this.duration),this.priority);events.push(event);};this._getEvents(date.startOf('day'),date.endOf('day'),iterator);//console.log("events =", JSON.stringify(events));
return events;}// This method returns events that start during the given time range, end
// during the time range, or encompass the time range.
// EXRULE is deprecated, so only process RDATE, RRULE, and EXDATE.
_getEvents(start,end,iterator){const ruleIterators=[];let ruleDateIndex=0;let exDateIndex=0;let exDate=null;let ruleDate=null;const from=start.minus(this.duration);let last=from;// RDATE
//const t0 = performance.now();
const ruleDates=this.recurrence.recurrenceOverrides;if(!this.recurrence.excluded&&ruleDates.length!==0){if(ruleDates[0]<last){ruleDateIndex=0;last=ruleDates[0];}else{ruleDateIndex=findIndex(ruleDates,last,compareDateTime);}ruleDate=ruleDates[ruleDateIndex];}//const t1 = performance.now();
//console.log('RDATE', (t1 - t0) / 1000);
// RRULE
for(const rule of this.recurrence.recurrenceRules){const it=rule.iterator(this.start,from);ruleIterators.push(it);it.next();// find first value.
}//const t2 = performance.now();
//console.log('RRULE', (t2 - t1) / 1000);
// EXDATE
if(this.recurrence.excluded&&ruleDates.length!==0){exDateIndex=findIndex(ruleDates,last,compareDateTime);exDate=ruleDates[exDateIndex];}//const t3 = performance.now();
//console.log('EXDATE', (t3 - t2) / 1000);
// Occurrences.
const findNextRuleIterator=()=>{if(ruleIterators.length===0){return null;}let nextIterator=null;for(const it of ruleIterators){if(it.done){continue;}if(nextIterator===null||it.value<nextIterator.value){nextIterator=it;}}return nextIterator;};const getNextRuleDay=()=>{ruleDate=ruleDates[++ruleDateIndex];};const getNextExDay=()=>{exDate=ruleDates[++exDateIndex];};const getNextOccurrence=()=>{while(true){let next=ruleDate;const it=findNextRuleIterator();if(next===null&&it===null){//console.log("no matches");
break;}let has_changed=false;if(next===null||it!==null&&it.value<next){//console.log("have rrule ...");
has_changed=true;// @ts-ignore: Object is possibly 'null'.
next=it.value;//console.log("value:", next?.toISO());
// @ts-ignore: Object is possibly 'null'.
it.next();}if(!has_changed){getNextRuleDay();}// @ts-ignore: Type 'null' is not assignable to type 'DateTime'.
last=next;if(exDate!==null){if(last>exDate){getNextExDay();}else if(last.equals(exDate)){continue;}}return last;}return null;};//console.log("loop start", last.toISO(), end.toISO());
while(last<end){const occ=getNextOccurrence();//console.log("occurrence:", occ && occ.toISO());
if(!occ)break;iterator(occ);}//console.log("loop end");
//const t4 = performance.now();
//console.log('occurrences', (t4 - t3) / 1000);
}toJSON(){return{id:this.id,start:this.start.toISO(),duration:this.duration.toISO(),priority:this.priority,recurrence:this.recurrence.toJSON()};}static fromJSON(json){const startTime=DateTime$1.fromISO(json.start);const endTime=startTime.plus(Duration.fromISO(json.duration));const recurrence=CalendarRecurrence.fromJSON(json.recurrence);const event=new CalendarEventSeries(json.id,json.parentId,json.data,startTime,endTime,json.priority,recurrence);return event;}}class CalendarRecurrence{constructor(){this.recurrenceRules=[];this.recurrenceOverrides=[];this.excluded=false;}addDailyRule(){const rule=new RecurrenceRule('daily');this.recurrenceRules.push(rule);return rule;}addDateExclusion(date){this.recurrenceOverrides.push(date);this.excluded=true;return this;}addDate(date){this.recurrenceOverrides.push(date);return this;}addHourlyRule(){const rule=new RecurrenceRule('hourly');this.recurrenceRules.push(rule);return rule;}addMinutelyRule(){const rule=new RecurrenceRule('minutely');this.recurrenceRules.push(rule);return rule;}addSecondlyRule(){const rule=new RecurrenceRule('secondly');this.recurrenceRules.push(rule);return rule;}addMonthlyRule(){const rule=new RecurrenceRule('monthly');this.recurrenceRules.push(rule);return rule;}addWeeklyRule(){const rule=new RecurrenceRule('weekly');this.recurrenceRules.push(rule);return rule;}addYearlyRule(){const rule=new RecurrenceRule('yearly');this.recurrenceRules.push(rule);return rule;}toJSON(){return{recurrenceRules:this.recurrenceRules.map(element=>element.toJSON()),recurrenceOverrides:this.recurrenceOverrides.map(element=>element.toISO()),excluded:this.excluded};}static fromJSON(json){const recurrence=new CalendarRecurrence();for(const rule of json.recurrenceRules){recurrence.recurrenceRules.push(RecurrenceRule.fromJSON(rule));}for(const override of json.recurrenceOverrides){recurrence.recurrenceOverrides.push(DateTime$1.fromISO(override));}recurrence.excluded=json.excluded;return recurrence;}}class NDay{constructor(day,nthOfPeriod){this.day=day;this.nthOfPeriod=nthOfPeriod;if(nthOfPeriod===0){throw new Error('nthOfPeriod must not be zero.');}}toJSON(){return typeof this.nthOfPeriod==='number'?{day:this.day,nthOfPeriod:this.nthOfPeriod}:{day:this.day};}static fromJSON(json){if('nthOfPeriod'in json){return new NDay(json.day,json.nthOfPeriod);}return new NDay(json.day);}}// Some components not supported in Google Apps Script.
//
...
class RecurrenceRule{constructor(frequency){this._cache=null;this._frequency=frequency;this._interval=1;//		this.#rscale = 'gregorian';
//		this.#skip = 'omit';
this._firstDayOfWeek='mo';this._byDay=[];this._byMonthDay=[];this._byMonth=[];this._byYearDay=[];this._byWeekNo=[];this._byHour=[];this._byMinute=[];this._bySecond=[];this._bySetPosition=[];this._timeZone=undefined;this._times=undefined;this._until=undefined;}toJSON(){return{frequency:this._frequency,interval:this._interval,//			rscale: this.#rscale,
//			skip: this.#skip,
firstDayOfWeek:this._firstDayOfWeek,byDay:this._byDay.map(day=>day.toJSON()),byMonthDay:this._byMonthDay,byMonth:this._byMonth,byYearDay:this._byYearDay,byWeekNo:this._byWeekNo,byHour:this._byHour,byMinute:this._byMinute,bySecond:this._bySecond,bySetPosition:this._bySetPosition,timeZone:this._timeZone,times:this._times,until:this._until?this._until.toISO():undefined};}static fromJSON(json){const recurrence=new RecurrenceRule(json.frequency);recurrence.interval(json.interval);if(typeof json.firstDayOfWeek==='string'&&json.firstDayOfWeek.length){recurrence.weekStartsOn(json.firstDayOfWeek);}if(Array.isArray(json.byDay)&&json.byDay.length){const byDay=[];for(const day of json.byDay){byDay.push(NDay.fromJSON(day));}recurrence.onlyOnWeekDays(byDay);}if(Array.isArray(json.byMonthDay)&&json.byMonthDay.length){recurrence.onlyOnMonthdays(json.byMonthDay);}if(Array.isArray(json.byMonth)&&json.byMonth.length){recurrence.onlyInMonths(json.byMonth);}if(Array.isArray(json.byYearDay)&&json.byYearDay.length){recurrence.onlyOnYearDays(json.byYearDay);}if(Array.isArray(json.byWeekNo)&&json.byWeekNo.length){recurrence.onlyOnWeeks(json.byWeekNo);}if(Array.isArray(json.byHour)&&json.byHour.length){recurrence.onlyOnHours(json.byHour);}if(Array.isArray(json.byMinute)&&json.byMinute.length){recurrence.onlyOnMinutes(json.byMinute);}if(Array.isArray(json.bySecond)&&json.bySecond.length){recurrence.onlyOnSeconds(json.bySecond);}if(Array.isArray(json.bySetPosition)&&json.bySetPosition.length){recurrence.bySetPosition(json.bySetPosition);}if(typeof json.timeZone==='string'&&json.timeZone.length){recurrence.setTimeZone(json.timeZone);}if(typeof json.times==='number'){recurrence.times(json.times);}if(typeof json.until==='string'&&json.until.length){recurrence.until(DateTime$1.fromISO(json.until));}return recurrence;}getFrequency(){return this._frequency;}getInterval(){return this._interval;}interval(interval){this._interval=interval;}getWeekStartsOn(){return this._firstDayOfWeek;}weekStartsOn(day){this._firstDayOfWeek=day;}getOnlyOnWeekDays(){return this._byDay;}onlyOnWeekDays(days){this._byDay=days;}getOnlyOnMonthDays(){return this._byMonthDay;}onlyOnMonthdays(days){this._byMonthDay=days;}getOnlyInMonths(){return this._byMonth;}onlyInMonths(months){this._byMonth=months;}getOnlyOnYearDays(){return this._byYearDay;}onlyOnYearDays(days){this._byYearDay=days;}getOnlyOnWeeks(){return this._byWeekNo;}onlyOnWeeks(weeks){this._byWeekNo=weeks;}getOnlyOnHours(){return this._byHour;}onlyOnHours(hours){this._byHour=hours;}getOnlyOnMinutes(){return this._byMinute;}onlyOnMinutes(minutes){this._byMinute=minutes;}getOnlyOnSeconds(){return this._bySecond;}onlyOnSeconds(seconds){this._bySecond=seconds;}bySetPosition(positions){this._bySetPosition=positions;}getSetPosition(){return this._bySetPosition;}getTimeZone(){return this._timeZone;}setTimeZone(timeZone){this._timeZone=timeZone;}getTimes(){return this._times;}times(times){this._times=times;}getUntil(){return this._until;}until(endDate){this._until=endDate;}isFinite(){return!!(this._times||this._until);}isByCount(){return!!(this._times&&!this._until);}iterator(dtstart,from){let options={};// RRule is somewhat broken and silently errors on empty or
// undefined options.
options['freq']=RRuleFrequencyFromString(this._frequency);options['dtstart']=dtstart.toJSDate();options['interval']=this._interval;if(this._firstDayOfWeek){options['wkst']=RRuleWeekDayFromString(this._firstDayOfWeek);}if(this._times){options['count']=this._times;}if(this._until){options['until']=this._until.toJSDate();}if(this._timeZone){options['tzid']=this._timeZone;}if(this._bySetPosition.length){options['bysetpos']=this._bySetPosition;}if(this._byMonth.length){options['bymonth']=this._byMonth;}if(this._byMonthDay.length){options['bymonthday']=this._byMonthDay;}if(this._byYearDay.length){options['byyearday']=this._byYearDay;}if(this._byWeekNo.length){options['byweekno']=this._byWeekNo;}if(this._byDay.length){options['byweekday']=this._byDay.map(element=>RRuleWeekDayFromString(element.day,element.nthOfPeriod));}if(this._byHour.length){options['byhour']=this._byHour;}if(this._byMinute.length){options['byminute']=this._byMinute;}if(this._bySecond.length){options['bysecond']=this._bySecond;}//		console.info(options);
let rrule;if(this._cache===null){rrule=new RRule(options,false);this._cache=rrule._cache;}else{rrule=new RRule(options,true/* no-cache */);rrule._cache=this._cache;}//		console.info(rrule.toString());
let date=from.toJSDate();let value=undefined;let done=false;const next=()=>{if(date===null){return;}//console.info("rrule.after(", date, ")");
date=rrule.after(date);//console.group();
//console.info("date:", date.toISOString());
value=date!==null?DateTime$1.fromJSDate(date):undefined;//console.info("value:", value && value.toISO());
done=value===null;//console.info("done:", done);
//console.groupEnd();
};const it={next,get value(){return value;},get done(){return done;}};return it;}}// vim: tabstop=8 softtabstop=0 noexpandtab shiftwidth=8 nosmarttab
console.info('CALENDAR: WebWorker started.');let calendar_event;// Certain objects need special attention to transfer over "Comlink",
// Luxon's DateTime is coded in milliseconds, ⚠️ losing timezone.
const DateTimeHandler={canHandle:val=>val instanceof DateTime$1,serialize:val=>{return[val.toMillis(),[]];},deserialize:num=>DateTime$1.fromMillis(num)};// CalendarEvent and CalendarEvent[] are flattened into JSON due to many Luxon
// DateTime instances within.
const CalendarEventHandler={canHandle:val=>val instanceof CalendarEvent,serialize:val=>{return[val.toJSON(),[]];},deserialize:obj=>CalendarEvent.fromJSON(obj)};const CalendarEventArrayHandler={canHandle:val=>Array.isArray(val)&&val.every(x=>x instanceof CalendarEvent),serialize:val=>{return[val.map(x=>x.toJSON()),[]];},deserialize:obj=>obj.map(x=>CalendarEvent.fromJSON(x))};transferHandlers.set("DATETIME",DateTimeHandler);transferHandlers.set("CALENDAREVENT",CalendarEventHandler);transferHandlers.set("CALENDAREVENTARRAY",CalendarEventArrayHandler);// The APIs exposed to the caller.
expose({parseSchedule,getEvents,prefetchEvents});// Create a CalendarEvent or CalendarEventSeries from JSON schedule.
function parseSchedule(parentId,json){if(json['@type']!=='Event'){console.warn(`CALENDAR: @type is not Event (${json['@type']})`,parentId,json);throw new Error('Invalid @type in schedule.');}console.log('CALENDAR: json',json);const startTime=DateTime$1.fromISO(json.start,{zone:json.timeZone});const endTime=startTime.plus(Duration.fromISO(json.duration));if('recurrenceRules'in json){const recurrence=new CalendarRecurrence();for(const rule of json.recurrenceRules){switch(rule.frequency){case'yearly':if('interval'in rule){recurrence.addYearlyRule().interval(rule.interval);}else{recurrence.addYearlyRule();}break;case'monthly':if('interval'in rule){recurrence.addMonthlyRule().interval(rule.interval);}else{recurrence.addMonthlyRule();}break;case'weekly':if('interval'in rule){recurrence.addWeeklyRule().interval(rule.interval);}else{recurrence.addWeeklyRule();}break;case'daily':if('interval'in rule){recurrence.addDailyRule().interval(rule.interval);}else{recurrence.addDailyRule();}break;case'hourly':if('interval'in rule){recurrence.addHourlyRule().interval(rule.interval);}else{recurrence.addHourlyRule();}break;case'minutely':if('interval'in rule){recurrence.addMinutelyRule().interval(rule.interval);}else{recurrence.addMinutelyRule();}break;case'secondly':if('interval'in rule){recurrence.addSecondlyRule().interval(rule.interval);}else{recurrence.addSecondlyRule();}break;}}calendar_event=new CalendarEventSeries(json.id,parentId,json.playlist,startTime,endTime,json.priority,recurrence);}else{calendar_event=new CalendarEvent(json.id,parentId,json.playlist,startTime,endTime,json.priority);}}// Find the events that exist between startTime and endTime for the parsed
// schedule.
