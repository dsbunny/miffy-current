...
const pos=this._active_media_assets.findIndex(x=>x.decl.id===id);const media_asset=this._active_media_assets[pos];this._media_list_duration=this._media_list_duration.minus({seconds:media_asset.duration});this._active_media_assets.splice(pos,1);}exposeNetwork(join,leave){this._join=join;this._leave=leave;}_parseSchedule(json){var _this10=this;return _asyncToGenerator(function*(){console.groupCollapsed("BASIC-SCHEDULER: _parseSchedule");if('cluster'in json&&typeof json.cluster==='object'&&typeof _this10._join==='function'){const cluster_as_text=JSON.stringify(json.cluster);if(typeof _this10._joined_cluster==='string'&&_this10._joined_cluster===cluster_as_text){console.log("BASIC-SCHEDULER: Already joined cluster.");}else if(typeof _this10._leave==='function'){yield _this10._leave();}yield _this10._join(json.cluster);_this10._joined_cluster=cluster_as_text;}else if(typeof _this10._leave==='function'){yield _this10._leave();_this10._joined_cluster=undefined;}_this10._transitionId=json.transition.id;_this10._transitionUrl=json.transition.href;_this10._transitionSize=json.transition.size;_this10._transitionHash=json.transition.hash;_this10._transitionIntegrity=json.transition.integrity;_this10._transitionMd5=json.transition.md5;_this10._transitionTime=json.transition.duration;const calendar_schedules=[];let trash_stack=[];// Copy umodified calendar schedules from running state.
const running_schedules_by_id=new Map();for(const schedule of _this10._calendar_schedules){running_schedules_by_id.set(schedule.id,schedule);}for(const decl of json.schedule){if(running_schedules_by_id.has(decl.id)){calendar_schedules.push(running_schedules_by_id.get(decl.id));running_schedules_by_id.delete(decl.id);}}trash_stack=trash_stack.concat(Array.from(running_schedules_by_id.values()));let promises=[];const now=DateTime$1.local();const lowWatermark={'seconds':30};let highWatermark={'seconds':90};const t0=performance.now();for(const decl of json.schedule){const schedule=new CalendarSchedule(decl,lowWatermark,highWatermark);promises.push(schedule.parseSchedule(decl));calendar_schedules.push(schedule);}yield Promise.all(promises);// preload events, ensuring calculation time is less than 60% of playback time.
...
break;}}const t3=performance.now();console.log(`Schedule parsed after ${(t3-t0)/1000}s.`);console.groupEnd();for(const schedule of calendar_schedules){schedule.prefetch(now);}_this10._calendar_schedules=calendar_schedules;// FIXME: Needs a close on complete flag to prevent interruption
// of playing content.
